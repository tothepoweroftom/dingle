...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"js/vendor/aframe-effects.min.js":[function(require,module,exports) {
(function (modules) {
  ...
  if (!window.AFRAME) {
    var Component = function Component(el, id) {
      var self = this;
      this.el = el;
      this.el;
      this.id = id;
    ...
    AFRAME.registerComponent = function (name, definition) {
      ...
      NewComponent = function NewComponent(el, attr, id) {
    ...
    AFRAME.registerSystem = function (name, definition) {
      ...
      NewSystem = function NewSystem(el, attr, id) {
    ...
    var fx = function fx(renderer, scene, cameras) {
      this.sceneEl = this;
  ...
  AFRAME.registerSystem("effects", {
    ...
    addLight: function addLight(behavior) {
      this.lightComponents.push(behavior);
    },
    removeLight: function removeLight(behavior) {
      var index = this.lightComponents.indexOf(behavior);
      this.lightComponents.splice(index);
    },
    setupPostState: function setupPostState() {
    ...
    renderPass: function renderPass(material, renderTarget, viewCb, forceClear) {
    ...
    materialize: function materialize(m) {
      var fs = ["uniform vec2 uvClamp;", "vec4 textureVR( sampler2D sampler, vec2 uv ) {", " return texture2D(sampler, vec2(clamp(uv.x, uvClamp.x, uvClamp.y), uv.y));", "} ", m.fragmentShader].join("\n");
        ...
        if (typeof obj === "string") {
          ...
          if (obj[0] === "#") {
            ...
            obj = {
              ...
              fragment: prop ? el[prop] instanceof Document ? el[prop].body.textContent : el[prop] : el.textContent,
    ...
    rebuild: function rebuild() {
      ...
      function pickup() {
        if (!temp.length) return;
        passes.push({
          pass: makepass(self.fuse(temp), true)
        });
        temp = [];
      }

      function makepass(material, _dispose, viewCb) {
        return {
          render: function render(renderer, writeBuffer, readBuffer) {
            self.renderPass(material, writeBuffer, viewCb);
          },
          dispose: function dispose() {
            if (_dispose) material.dispose();
          }
        };
      }
    ...
    isActive: function isActive(behavior, resize) {
    ...
    register: function register(behavior) {
      this.effects[behavior.attrName] = behavior;
      this.needsUpdate = true;
    },
    unregister: function unregister(behavior) {
    ...
    tick: function tick(time, timeDelta) {
      ...
      this.cameras = [];
      this.time.value = time / 1e3;
      this.timeDelta.value = timeDelta / 1e3;
    ...
    resizeTiles: function resizeTiles() {
    ...
    tileLights: function tileLights(renderer, scene, camera) {
  ...
  var BlurDirectionX = new THREE.Vector2(1, 0);
  var BlurDirectionY = new THREE.Vector2(0, 1);
  ...
  AFRAME.registerComponent("colors", {
    ...
    schema: {
      ...
      quant: {
        type: "vec3",
        default: new THREE.Vector3(0.2, 0.2, 0.2)
      },
      orig: {
        type: "vec3",
        default: new THREE.Vector3(1, 1, 1)
      ...
      green: {
        type: "vec3",
        default: new THREE.Vector3(0, 0.5, 0.5)
      },
      blue: {
        type: "vec3",
        default: new THREE.Vector3(0, 0.5, 0.5)
    ...
    rebuild: function rebuild() {
      ...
      this.fragment = [this.data.lutClamp ? "" : "#define $LUT_NO_CLAMP 1", this.data.lutFlip ? "#define $LUT_FLIP_Y 1" : "", this.preFragment, arr.join("\n"), "}"].join("\n");
  ...
  AFRAME.registerComponent("godrays", {
    ...
    tock: function tock() {
      ...
      var fn = function fn(material, camera, eye) {
  ...
  AFRAME.registerComponent("ssao", {
    ...
    rebuild: function rebuild(sao, blur) {
      var d = this.data;
  ...
  THREE.BlurShaderUtils = {
    ...
    configure: function configure(kernelRadius, stdDev, uvIncrement) {
      return {
  ...
  module.exports = function (radius, stdDev, uvIncrement) {
    ...
    return {
      ...
      uniforms: {
        ...
        sampleUvOffsets: {
          type: "v2v",
          value: config.sampleUvOffsets
        },
        sampleWeights: {
          type: "1fv",
          value: config.sampleWeights
        },
        depthCutoff: {
          type: "f",
          value: 10
  ...
  AFRAME.registerShader("standard-fx", {
    ...
    updateEnvMap: function updateEnvMap(data) {
      var self = this;
      var material = this.material;
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","js/vendor/aframe-effects.min.js"], null)
//# sourceMappingURL=/aframe-effects.min.df60e234.js.map